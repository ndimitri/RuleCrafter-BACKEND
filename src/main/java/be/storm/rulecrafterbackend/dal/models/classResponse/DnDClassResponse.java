...
public class DnDClassResponse {
  ...
  public ClassType mapToClassType(DnDClassResponse response) {
    ClassType classType = new ClassType();

    // Mappe les champs simples
    classType.setName(response.getName());
    classType.setHitPointDie(response.getHit_die());

    // Mappe le champ primaryAbility (par exemple, STR)
    Ability primaryAbility = Ability.getByApiIndex(response.getSaving_throws().get(0).getIndex());
    classType.setPrimaryAbility(primaryAbility);
